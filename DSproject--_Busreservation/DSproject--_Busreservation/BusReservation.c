    ...
    while (presentnode) {
        if (presentnode->PassnNo == s) {
            *custIDmatched = 1;
            ...
            printf("\n||              TICKET COST: \tBdt.%d                             ||", cost(presentnode));
            printf("\n-----------------------------------------------------------------");
            resetColor();
...
// Function to disable echoing and get a character
char getch() {
...
login:
    ...
    printf("\nPassWord: ");
    // Read the password without echoing characters
    int i = 0;
    char ch;
    while ((ch = getch()) != '\n') { // '\n' is the Enter key in Unix-like systems
        if (i < 9) { // ensure we don't overflow the buffer
            matchPass[i] = ch;
            i++;
        }
    }
    matchPass[i] = '\0'; // Null-terminate the string

    value = strcmp(passWord, matchPass); // string compare is function defined in header file i.e string.h
    if (value != 0) {
        redColor();
...
void busLists()
{
  ...
  printf("-----------------------------------------------------------------------------------------");
  resetColor();
  printf("\n1\tShadhin         \tRampura to Mirpur       \tBdt.70   \t\t07:00  PM");
  printf("\n2\tRamjan     \t\tRampura To Mohammadpur       \tBdt.60    \t\t01:30  PM");
  printf("\n3\tTaranga Plus   \t\tStuff Quarter To Mohammadpur\tBdt.40    \t\t03:50  PM");
  printf("\n4\tAkash         \t\tBanasree To Kawranbazar    \tBdt.70    \t\t01:00  PM");
  printf("\n5\tUttara Poribahan     \tUttara To Motijheel \t\tBdt.55    \t\t12:05  PM");
  printf("\n6\tGulshan Chaka     \tHatirjheel to Gulshan-2    \tBdt.40    \t\t09:30  PM");
  printf("\n7\tDhaka Chaka        \tGulshan-2 To Dhanmandi         \tBdt.70   \t\t11:00  PM");
  printf("\n8\tDhaka Poribohon       \tUttara To Gulshan-2     \tBdt.55    \t\t08:15  PM");
  printf("\n9\tBRTA Poribahan        \tDhanmandi To Uttara       \tBdt.40    \t\t04:00  PM");
  printf("\n");
